...
if(isset($_POST['scan_refresh'])) {
	...
	$np = scan_for_plugins();
	$bBlog->smartyObj->assign('np',"<b style='color: red;'>$np</b><br />");
}
...
if($p && is_array($plugins['admin'][$p])) { // successful call to plugin
	...
	$bBlog->smartyObj->assign('plugin',$plugins['admin'][$p]);
	$bBlog->smartyObj->assign('plugin_template','plugins/'.$plugins['admin'][$p]['template']);
	$bBlog->smartyObj->assign('title',$plugins['admin'][$p]['displayname']);
	include_once BBLOGROOT.'bBlog_plugins/'.$plugins['admin'][$p]['file'];
        $func = "admin_plugin_".$p."_run";
        $func($bBlog);
}

$bBlog->smartyObj->assign('plugin_ar',$plugin_ar);
$bBlog->smartyObj->assign('show_plugin_menu',$show_plugin_menu);
