...
class bBlog {
	...
	**********************************************************************/
	...
	function prep_post($post) {
        ...
        $npost['body'] = htmlspecialchars($npost['body']);
	...
	*/
	...
	function new_comment($postid, $replyto = 0) {
		...
		if (!$post) {
			...
			$this->template_message("Error adding comment", "couldn't find post id $postid");
		}
		elseif ($post->allowcomments == ('disallow') or ($post->allowcomments == 'timed' and $post->autodisabledate < time())) {
			$this->template_message("Error adding comment", "Comments have been turned off for this post");
		} else {
			...
			setcookie("postername", $postername, $ctime);
			setcookie("posteremail", $posteremail, $ctime);
			setcookie("posterwebsite", $posterwebsite, $ctime);
			...
			if (C_COMMENT_TIME_LIMIT > 0) {
				...
				if ($this->num_rows > 0) {
					$this->template_message("Comment Flood Protection", "Error adding comment. You have tried to make a comment too soon after your last one. Please try again later. This is a bBlog spam prevention mesaure");

				}

			}
			if(C_IMAGE_VERIFICATION == 'true') {
				if(!$this->authimage->checkAICode($_POST['spamcode'])){
					$_SESSION['postercomment'] = $comment;
					$this->template_message("Spam prevention", "Error adding comment. Please check that the characters you typed for the image verification are correct");
					//$this->template_message("Prevence spamu", "Chyba při vkládání komentáře. Opište prosím přesně text na obrázku do polí�?ka pod ním. Děkujeme.<br /><input type='button' value='Zpět' onclick='history.go(-1)' />");
				}
			}
			...
			if ($insid < 1) {
				$this->template_message("Error", "Error inserting comment : ".mysql_error());
			} else {
				// notify
